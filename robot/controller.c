 ...
 */
void controller_connect(struct controller *ctrl) {
  DIR *dp;
  struct dirent *ent;
  ...
  while ((ent = readdir(dp)))
    if (strstr(ent->d_name, JS_PREFIX)) {
      ...
      ctrl->buttons = 0;
      ctrl->axes = 0;
      memset(&ctrl->A, 0, (size_t)ctrl + sizeof(struct controller) - (size_t)&ctrl->A);
      break;
    }
  if (!ctrl->connected)
    goto error;
...
error:
  fprintf(stderr, "Error: Cannot connect to ctrl\n");
  ctrl->connected = 0;
  if (ctrl->fd != -1)
    close(ctrl->fd);
  ctrl->fd = -1;
 ...
 *    A pointer to a controller struct.
 */
void controller_update(struct controller *ctrl) {
 ...
 *    A pointer to a controller struct.
 */
void controller_disconnect(struct controller *ctrl) {
  /* clean up */
  if (ctrl->fd != -1)
    close(ctrl->fd);
  if (ctrl->name != NULL)
